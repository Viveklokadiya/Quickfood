...
router.get("/api/stripe/order", async (req, res) => {
  ...
  try {
    ...
    const existingOrder = await ordersession.findOne({ id: orderData.id });

    if (existingOrder) {
      // If the order with the same ID exists, update it
      // await ordersession.updateOne({ id: orderData.id }, orderData);
      // alert("order exist");
      // res.send({ message: "order exist" });
      console.log("order exist", orderData.id);
    } else {
      // If the order with the same ID doesn't exist, insert it
      const newOrder = new ordersession(orderData);
      await newOrder.save();
    }
