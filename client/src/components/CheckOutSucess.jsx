...
const CheckOutSucess = () => {
  ...
  useEffect(() => {
    axios.delete(`http://3.109.200.232:6001/CartReset/${email}`)
    ...
    const fetchOrderData = async () => {
      try {
        ...
        const response = await axios.get(`http://3.109.200.232:6001/SuccessfulPayment/api/stripe/order?session_id=${sessionId}`); // Replace '/api/stripe/order' with your backend endpoint for fetching order data
        setOrderData(response.data);
        setIsLoading(false);
        const emlrs = await axios.get(`http://3.109.200.232:6001/email/send/${response.data.id}`);
        console.log(emlrs);
      } catch (error) {
